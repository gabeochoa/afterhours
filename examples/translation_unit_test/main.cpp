...
struct RenderSystem : System<Transform> {
  void for_each_with(Entity &entity, Transform &transform,
  ...
  void for_each_with(const Entity &entity, const Transform &transform,
...
int main() {
  ...
  std::cout
      << "This test demonstrates the static initialization order fiasco\n";
  ...
  std::cout << "By defining components in a single translation unit "
               "(components.h),\n";
  std::cout << "all systems now use the same component type IDs!\n";
  std::cout << "This ensures that systems can find entities because their "
               "component\n";

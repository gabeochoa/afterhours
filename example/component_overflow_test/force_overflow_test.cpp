...
int main() {
    ...
    std::cout << "This test directly calls the internal ID generator to force "
                 "overflow"
              << std::endl;
    std::cout << "Component limit: " << max_num_components << std::endl
              << std::endl;
    ...
    for (size_t i = 0; i < max_num_components + 5; i++) {
        ...
        if (i < 5 || i >= max_num_components - 2) {
            ...
            std::cout << "... (generating " << (max_num_components - 7)
                      << " more IDs) ..." << std::endl;
        }

        // Verify ID is always within bounds
        if (id >= max_num_components) {
            std::cout << "❌ CRITICAL ERROR: ID " << id << " exceeds limit "
                      << max_num_components << std::endl;
            std::cout << "This could cause array bounds violations!"
                      << std::endl;
            return 1;
        }
    ...
    if (overflow_detected) {
        ...
        std::cout << "   ✓ IDs were clamped to safe values when limit exceeded"
                  << std::endl;
        ...
        std::cout << "   This might be because we didn't reach the actual limit"
                  << std::endl;
    ...
    std::cout << "        // log_error(...) - COMMENTED OUT & BROKEN"
              << std::endl;
    std::cout << std::endl;

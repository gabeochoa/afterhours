...
int main() {
    ...
    std::cout << "Testing all log levels and formatting..." << std::endl
              << std::endl;
    ...
    log_error("Formatted error: Failed operation at line %d in file %s", 42,
              "test.cpp");
    ...
    log_info("Successfully registered LogTestComponent with ID %zu",
             component_id);

    // Verify the component ID is valid
    if (component_id < max_num_components) {
        log_info(
            "Component ID validation passed - ID %zu is within bounds [0, %zu)",
    ...
    log_info("Multi-arg test: %s has %d components with limit %zu", "System", 5,
             max_num_components);

    std::cout << std::endl << "=== Test Results ===" << std::endl;
    std::cout << "✅ INFO logging: Working correctly (outputs to stdout)"
              << std::endl;
    std::cout << "✅ WARN logging: Working correctly (outputs to stdout)"
              << std::endl;
    std::cout << "✅ ERROR logging: Working correctly (outputs to stderr)"
              << std::endl;
    std::cout << "✅ Formatted logging: Working correctly with printf-style "
                 "formatting"
              << std::endl;
    std::cout << "✅ Integration: Logging system integrates properly with "
                 "component system"
              << std::endl;

    std::cout << std::endl
              << "=== Comparison with Previous State ===" << std::endl;
    std::cout << "BEFORE: log_error(...) {} - Empty function, no output"
              << std::endl;
    std::cout
        << "AFTER:  log_error(...) - Functional logging with proper formatting"
        << std::endl;
    std::cout << std::endl;
    std::cout << "The logging system is now fully functional and ready for "
                 "production use!"
              << std::endl;

...
int main(int, char **) {
    ...
    if (path.empty()) {
        ...
        if (path.size() > 5) {
            std::cout << "  ... (" << (path.size() - 5) << " more steps)" << std::endl;
        }
    }
    ...
    pathfinding::request_path(entity.id, Vec2{0.0f, 0.0f}, Vec2{3.0f, 3.0f},
        [](const std::deque<Vec2> &path) {
            std::cout << "Async path received with " << path.size() << " steps" << std::endl;
        });
    ...
    std::cout << "\nPathfinding plugin example completed successfully!" << std::endl;
    return 0;
}

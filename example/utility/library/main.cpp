...
struct Sound {
    ...
    Sound(const std::string &fname, float dur)
        : filename(fname), duration(dur) {}
};
...
int main() {
    ...
    try {
        ...
        std::cout << "  Error getting player texture: " << e.what()
                  << std::endl;
    }
    ...
    if (!duplicateResult) {
        std::cout << "  Correctly rejected duplicate 'player_idle'"
                  << std::endl;
    }

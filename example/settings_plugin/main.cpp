...
int main(int, char **) {
    ...
    std::cout << "  Fullscreen: " << (settings_data.fullscreen ? "Yes" : "No")
              << std::endl;
    ...
    if (settings::load<GameSettings>()) {
        ...
        std::cout << "  Volume: " << settings_data.volume << std::endl;
        std::cout << "  Player Name: " << settings_data.player_name
                  << std::endl;
        std::cout << "  Fullscreen: "
                  << (settings_data.fullscreen ? "Yes" : "No") << std::endl;
    } else {
        std::cout << "\nFailed to load settings!" << std::endl;
    }

    std::cout << "\nSettings plugin example completed successfully!"
              << std::endl;
    return 0;
}

...
int main(int, char **) {
    ...
    entity1.addComponent<timer::HasTimer>(2.0f,
                                          true);  // 2 second timer, auto-reset
    ...
    for (int frame = 0; frame < 50; ++frame) {
        ...
        if (entity1.has<timer::HasTimer>()) {
            ...
            if (timer.is_ready()) {
                std::cout << "Frame " << frame << " (" << total_time
                          << "s): Entity 1 timer triggered!" << std::endl;
            }
        }
        ...
        if (entity2.has<timer::TriggerOnDt>()) {
            ...
            if (trigger.test(dt)) {
                std::cout << "Frame " << frame << " (" << total_time
                          << "s): Entity 2 trigger fired!" << std::endl;
            }
        }

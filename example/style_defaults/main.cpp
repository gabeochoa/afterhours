...
struct StylingDefaultsDemo : System<UIContext<InputAction>> {
    virtual void for_each_with(Entity &entity, UIContext<InputAction> &context,
        ...
        printf(
            "✓ Set up styling defaults for Button, Div, and Slider "
            "components\n");
        ...
        auto test_slider =
            imm::slider(context, mk(container.ent()), slider_value,
                        ComponentConfig{}.with_label("Test Slider"));
        printf(
            "✓ Created slider with minimal config - using Slider defaults\n");
        ...
        printf(
            "✓ Builder pattern works - all methods inherited automatically\n");
        ...
        auto custom_button = imm::button(
            context, mk(container.ent()),
            ComponentConfig{}
                .with_label("Custom Button")
                .with_size(ComponentSize{pixels(180.f), pixels(45.f)})
                .with_color_usage(Theme::Usage::Secondary));
        printf(
            "✓ Created custom button with partial override - merged with "
            "defaults\n");

...
TEST_CASE("top padding") {
    ...
    // Note: Current layout behavior - sibling positioning uses child's rect height (50),
    // not bounds height (60). Padding doesn't push siblings down.
...
TEST_CASE("vertical padding") {
    ...
    // Note: Current layout behavior - sibling positioning uses child's rect height (50),
    // not bounds height (70). Padding doesn't push siblings down.
...
TEST_CASE("left percent margin on grandparent") {
    ...
    // gap = 10% of parent width (1280) = 128px
    // Since grandparent only has 100px desired width, margin exceeds element size
    // Layout clamps rect width to 0 when margin > desired width

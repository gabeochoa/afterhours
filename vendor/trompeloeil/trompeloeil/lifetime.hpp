...
class deathwatched : public T {
   ...
   public:
    ...
    trompeloeil::lifetime_monitor*& trompeloeil_expect_death(
        trompeloeil::lifetime_monitor* monitor) const noexcept {
        auto lock = get_lock();
   ...
   private:
    mutable null_on_move<trompeloeil::lifetime_monitor>
...
struct lifetime_monitor : public expectation {
    ...
    lifetime_monitor(::trompeloeil::deathwatched<T> const& obj,
                     char const* obj_name_, char const* invocation_name_,
                     char const* call_name_, location loc_) noexcept
    ...
    bool is_satisfied() const noexcept override { return died; }

    bool is_saturated() const noexcept override { return died; }
    ...
    void set_sequence(T&&... t) {
        ...
        auto seq = detail::make_unique<handler>(*sequences, invocation_name,
                                                loc, std::forward<T>(t)...);
...
deathwatched<T>::~deathwatched() {
    ...
    std::ostringstream os;
    os << "Unexpected destruction of " << TROMPELOEIL_TYPE_ID_NAME(T) << "@"
       << this << '\n';
...
struct lifetime_monitor_releaser {
    template<bool b>
    std::unique_ptr<trompeloeil::lifetime_monitor> operator+(
        lifetime_monitor_modifier<b>&& m) const {
        return std::move(m);
    }
};
...
#define TROMPELOEIL_REQUIRE_DESTRUCTION_(obj, obj_s)              \
    std::unique_ptr<trompeloeil::expectation> TROMPELOEIL_CONCAT( \
        trompeloeil_death_monitor_, __LINE__) =                   \
...
#define TROMPELOEIL_NAMED_REQUIRE_DESTRUCTION_(prefix, obj, obj_s)         \
    trompeloeil::lifetime_monitor_releaser{} +                             \
        ...
        ::trompeloeil::detail::make_unique<trompeloeil::lifetime_monitor>( \
            obj, obj_s, prefix "REQUIRE_DESTRUCTION(" obj_s ")",           \

...
class duck_typed_matcher : public matcher {
   public:
    ...
    template<typename V, typename = detail::enable_if_t<!is_matcher<V>{}>,
...
template<typename Predicate, typename Printer, typename MatcherType,
         typename... T>
class predicate_matcher : private Predicate,
                          private Printer,
                          public MatcherType {
   public:
    template<typename... U>
    constexpr predicate_matcher(
        Predicate&& pred, Printer&& printer,
        U&&... v) noexcept(noexcept(std::tuple<T...>(std::declval<U>()...)) &&
                           noexcept(Predicate(std::declval<Predicate&&>())) &&
                           noexcept(Printer(std::declval<Printer&&>())))
    ...
    constexpr bool matches(V&& v) const
        noexcept(noexcept(std::declval<Predicate const&>()(
            std::declval<V&&>(), std::declval<const T&>()...))) {
   ...
   private:
    ...
    template<size_t... I>
    std::ostream& print_(std::ostream& os_,
...
template<typename MatchType, typename Predicate, typename Printer,
         typename... T>
using make_matcher_return = predicate_matcher<
    Predicate, Printer,
...
template<typename MatchType, typename Predicate, typename Printer,
         typename... T>
inline make_matcher_return<MatchType, Predicate, Printer, T...> make_matcher(
    Predicate pred, Printer print, T&&... t) {

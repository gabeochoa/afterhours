...
class sequence_type {
   public:
    ...
    unsigned cost(sequence_matcher const* m) const noexcept;

    void retire_until(sequence_matcher const* m) noexcept;

    void add_last(sequence_matcher* m) noexcept;

    void validate_match(severity s, sequence_matcher const* matcher,
                        char const* seq_name, char const* match_name,
                        location loc) const;
...
class sequence_matcher : public list_elem<sequence_matcher> {
   public:
    ...
    sequence_matcher(char const* exp, location loc,
                     const sequence_handler_base& handler, init_type i) noexcept
        : seq_name(i.first),
          exp_name(exp),
          exp_loc(loc),
    ...
    void validate_match(severity s, char const* match_name,
                        location loc) const {
        seq.validate_match(s, this, seq_name, match_name, loc);
    }

    unsigned cost() const noexcept { return seq.cost(this); }

    bool is_satisfied() const noexcept;

    bool is_optional() const noexcept;

    void retire() noexcept { this->unlink(); }

    void retire_predecessors() noexcept { seq.retire_until(this); }

    void print_expectation(std::ostream& os) const {
        os << exp_name << " at " << exp_loc;
    }

    char const* sequence_name() const noexcept { return seq_name; }
...
inline bool sequence_type::is_completed() const noexcept {
...
inline bool sequence_type::is_first(sequence_matcher const* m) const noexcept {
    return !matchers.empty() && &*matchers.begin() == m;
}

inline unsigned sequence_type::cost(sequence_matcher const* m) const noexcept {
...
inline void sequence_type::retire_until(sequence_matcher const* m) noexcept {
...
inline void sequence_type::validate_match(severity s,
...
inline sequence_type::~sequence_type() {
    ...
    while (!matchers.empty()) {
        ...
        if (!touched) {
            os << "Sequence expectations not met at destruction of sequence "
                  "object \""
               << m->sequence_name() << "\":";
            touched = true;
        }
...
inline void sequence_type::add_last(sequence_matcher* m) noexcept {
    matchers.push_back(m);
}

inline bool sequence_matcher::is_satisfied() const noexcept {
    return sequence_handler.is_satisfied();
}

inline bool sequence_matcher::is_optional() const noexcept {
    return sequence_handler.get_min_calls() == 0;
}

template<size_t N>
struct sequence_matchers {
    void validate(severity s, char const* match_name, location loc) {
        for (auto& e : matchers) {
...
#define TROMPELOEIL_IN_SEQUENCE(...)       \
    in_sequence(TROMPELOEIL_INIT_WITH_STR( \
        ::trompeloeil::sequence_matcher::init_type, __VA_ARGS__))

...
class ptr_deref : public matcher {
   public:
    template<typename U,
             typename = decltype(can_match_parameter<detail::remove_reference_t<
    ...
    friend std::string param_name_prefix(const ptr_deref*) {
...
inline ::trompeloeil::ptr_deref<detail::decay_t<M>> operator*(M&& m) {

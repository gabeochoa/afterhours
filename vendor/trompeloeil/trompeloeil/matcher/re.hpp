...
struct regex_check {
    class string_helper  // a vastly simplified string_view type of class
    {
       public:
        template<typename S,
                 typename = decltype(std::declval<const char*&>() =
                                         std::declval<S>().data()),
                 typename = decltype(std::declval<S>().length())>
        string_helper(const S& s) noexcept
            : begin_(s.data()), end_(begin_ + s.length()) {}

        constexpr string_helper(char const* s) noexcept
            : begin_(s), end_(s ? begin_ + strlen(s) : nullptr) {}

        constexpr explicit operator bool() const { return begin_; }
    ...
    regex_check(std::regex&& re_,
        ...
        : re(std::move(re_)), match_type(match_type_) {}
...
template<typename Kind = wildcard,
         typename R = make_matcher_return<
             Kind, lambdas::regex_check, lambdas::regex_printer, std::string&&>>
auto re(std::string s,
    ...
    return make_matcher<Kind>(
        lambdas::regex_check(std::regex(s, opt), match_type),
        lambdas::regex_printer(), std::move(s));
}

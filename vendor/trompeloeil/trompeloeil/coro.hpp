...
struct coro_value_type {
    static auto func() {
        if constexpr (requires { std::declval<T>().operator co_await(); }) {
            return type_wrapper<decltype(std::declval<T>()
                                             .operator co_await()
                                             .await_resume())>{};
        } else {
...
struct yield_expr_base<Sig, true>
    : public list_elem<yield_expr_base<Sig, true>> {
    ~yield_expr_base() override = default;

    using ret_type = coro_value_type_t<return_of_t<Sig>>;
    virtual ret_type expr(call_params_type_t<Sig>&) const = 0;
};

template<typename Sig>
struct yield_expr_base<Sig, false>
    : public list_elem<yield_expr_base<Sig, false>> {
...
struct yield_expr : yield_expr_base<Sig> {
    ...
    template<typename E>
    explicit yield_expr(E&& e_) : e(std::forward<E>(e_)) {}

    ret_type expr(call_params_type_t<Sig>& t) const override { return e(t); }
...
struct co_throw_handler_t {
    ...
    explicit co_throw_handler_t(H&& h_) : h(std::move(h_)) {}
...
struct handle_co_yield {
    ...
    static call_modifier<Matcher, modifier_tag, Parent> action(
        call_modifier<Matcher, modifier_tag, Parent>&& m, E&& e) {
        ...
        constexpr bool is_matching_type = std::invoke([] {
            if constexpr (is_coroutine && !is_void) {
                using promise =
                    typename std::coroutine_traits<sigret>::promise_type;
        ...
        static_assert(!is_coroutine || !is_void, "You cannot CO_YIELD void");
        ...
        if constexpr (valid) {
            if (!m.matcher->yield_expressions) {
                m.matcher->yield_expressions =
                    std::make_shared<yield_expr_list<signature>>();
            }
...
struct handle_co_return {
    ...
    static call_modifier<
        ...
        co_return_injector<return_of_t<typename Parent::signature>, Parent>>
    action(call_modifier<Matcher, modifier_tag, Parent>&& m, H&& h) {
        ...
        constexpr bool is_matching_type = std::invoke([] {
            if constexpr (is_coroutine) {
                ...
                if constexpr (std::is_same_v<void, ret>) {
                    ...
                    return requires(promise p) {
                        p.return_value(std::declval<ret>());
                    };
                }
            }
            return true;
        ...
        static_assert(!has_return, "CO_RETURN and RETURN cannot be combined");
        static_assert(has_return || !has_co_return,
                      "Multiple CO_RETURN does not make sense");
        static_assert(has_return || is_coroutine,
                      "CO_RETURN when return type is not a coroutine");
        static_assert(
            has_return || has_co_return || !is_coroutine || is_matching_type,
        ...
        constexpr bool valid = !has_return && !has_co_return && is_coroutine &&
                               is_matching_type && !Parent::throws &&
                               Parent::upper_call_limit > 0;
        if constexpr (valid) {
            ...
            m.matcher->return_handler_obj.reset(
                new handler(std::forward<H>(h), m.matcher->yield_expressions));
        }
        return {std::move(m).matcher};
    }
};
...
struct handle_co_throw {
    ...
    static call_modifier<Matcher, modifier_tag, throw_injector<Parent>> action(
        ...
        if constexpr (valid) {
            ...
            m.matcher->return_handler_obj.reset(new ret_handler(
                std::move(handler), m.matcher->yield_expressions));
        }
        return {std::move(m).matcher};
    }
};

...
struct HeadlessGLMacOS {
  ...
  inline bool init(int width, int height) {
    (void)width;
    (void)height; // Context is resolution-independent
    ...
    if (version) {
      ...
      fprintf(stderr,
              "HeadlessGL: Warning - glGetString(GL_VERSION) returned NULL\n");
    }
  ...
  inline void make_current() { CGLSetCurrentContext(context); }

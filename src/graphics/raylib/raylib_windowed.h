...
struct RaylibWindowed {
    ...
    /// Creates a resizable window with MSAA and sets up a render texture for consistent capture.
    /// @return true if initialization succeeded, false if window or render texture creation failed.
    ...
    RenderTextureType& get_render_texture() {

...
struct HasSprite : BaseComponent {
    ...
    float scale;
    Color colorTint;
    HasSprite(const Vector2Type pos, const Vector2Type size_,
              const float angle_, const Rectangle frm, const float scl,
              const Color colorTintIn)
        : transform(pos, size_, angle_),
          frame{frm},
          scale{scl},
    ...
    auto &update_transform(const Vector2Type &pos, const Vector2Type &size_,
                           const float &ang) {
        return transform.update_position(pos).update_size(size_).update_angle(
            ang);
    }
...
struct HasAnimation : BaseComponent {
    ...
    HasAnimation(const Params &params_in)
        : transform(params_in.position, params_in.size, params_in.angle),
          params(params_in),
          cur_frame_position(params_in.start_position),
...
struct AnimationUpdateCurrentFrame : System<HasAnimation> {
    virtual void for_each_with(Entity &entity, HasAnimation &hasAnimation,
        ...
        const auto [i, j] = idx_to_next_sprite_location(
            (int) hasAnimation.cur_frame_position.x,

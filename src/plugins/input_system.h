...
struct input : developer::Plugin {
    ...
    static bool is_mouse_button_pressed(const MouseButton button) {
        ...
        return testing::test_input::is_mouse_button_pressed(
            button, [](int b) -> bool {
                // Manual edge detection: raylib IsMouseButtonPressed is broken
                // on macOS because glfwSwapBuffers pumps the Cocoa event queue,
                // setting currentButtonState before PollInputEvents copies
                // current→previous. Use IsMouseButtonDown + edge tracking
                // instead.
                static bool prev[8] = {};
    ...
    static void try_load_gamepad_mappings() {
        if (!raylib::FileExists("resources/gamecontrollerdb.txt")) return;
        char *data = raylib::LoadFileText("resources/gamecontrollerdb.txt");
        if (!data) return;
        int result = raylib::SetGamepadMappings(data);
        raylib::UnloadFileText(data);
        log_info("Loaded gamepad mappings ({} entries)", result);
    }
    ...
    struct ActionDone {
        ...
        ActionDone(DeviceMedium m, GamepadID i, int a, float amount,
                   float length, KeyCode key = 0, uint8_t mods = 0,
            ...
            : medium(m),
              ...
              has_explicit_modifiers(explicit_mods) {}
    };
    ...
    // actions that matched the same physical key. E.g., if Shift+LEFT fires
    // TextSelectLeft (exclusive), don't also fire WidgetLeft (permissive LEFT).
    static void suppress_permissive_duplicates(std::vector<ActionDone> &actions) {
        // Collect keys claimed by exclusive chord matches
        std::vector<KeyCode> claimed_keys;
        for (const auto &ad : actions) {
            if (ad.medium == DeviceMedium::Keyboard &&
                ad.has_explicit_modifiers && ad.matched_key != 0) {
                claimed_keys.push_back(ad.matched_key);
            }
        }
        if (claimed_keys.empty()) return;

        std::erase_if(actions, [&](const ActionDone &ad) {
            if (ad.medium != DeviceMedium::Keyboard || ad.has_explicit_modifiers)
                return false;
            for (KeyCode k : claimed_keys) {
                if (ad.matched_key == k) return true;
            }
            return false;
        });
    }
...
struct layered_input : developer::Plugin {
    ...
    static void enforce_singletons(SystemManager &sm) {
        sm.register_update_system(
            std::make_unique<
                developer::EnforceSingleton<input::InputCollector>>());
        sm.register_update_system(
            std::make_unique<
                developer::EnforceSingleton<input::ProvidesMaxGamepadID>>());
        // Note: Can't enforce ProvidesLayeredInputMapping without knowing
        // LayerEnum at compile time
    }
};

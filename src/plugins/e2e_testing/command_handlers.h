...
struct HandleTypeCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("type")) return;
        if (!cmd.has_args(1)) {
...
struct HandleKeyCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("key")) return;
        if (cmd.args.empty()) {
...
struct HandleClickCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("click")) return;
        if (!cmd.has_args(2)) {
...
struct HandleDoubleClickCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("double_click")) return;
        if (!cmd.has_args(2)) {
...
struct HandleDragCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("drag")) return;
        if (!cmd.has_args(4)) {
...
struct HandleMouseMoveCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("mouse_move")) return;
        if (!cmd.has_args(2)) {
...
struct HandleMouseDownCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("mouse_down")) return;
        if (!cmd.has_args(2)) {
...
struct HandleMouseUpCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("mouse_up")) return;
...
struct HandleScrollWheelCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("scroll_wheel")) return;
        if (!cmd.has_args(2)) {
...
struct HandleDragToCommand : System<PendingE2ECommand> {
    ...
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("drag_to")) return;
        if (!cmd.has_args(4)) {
...
struct HandleWaitCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("wait")) return;
...
struct HandleWaitFramesCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("wait_frames")) return;
        cmd.consume();  // Wait timing is handled by E2ERunner
    }
};
...
struct HandleExpectNoTextCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("expect_no_text")) return;
        if (cmd.args.empty()) {
...
struct HandleScreenshotCommand : System<PendingE2ECommand> {
    ...
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("screenshot")) return;
...
struct HandleResetTestStateCommand : System<PendingE2ECommand> {
    ...
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
                               float) override {
        if (cmd.is_consumed() || !cmd.is("reset_test_state")) return;
...
struct HandleResizeCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
        ...
        // Update the ECS resolution singleton (authoritative source for UI
        // layout)
        auto *pcr = EntityHelper::get_singleton_cmp<
            window_manager::ProvidesCurrentResolution>();
        if (pcr) {
...
struct HandleAssertNoOverflowCommand : System<PendingE2ECommand> {
    virtual void for_each_with(Entity &, PendingE2ECommand &cmd,
        ...
        // Don't use force_merge: only check entities that completed layout
        // pipeline
        ...
        for (Entity &entity : all_ui) {
            ...
            if (!cmp.was_rendered_to_screen || cmp.should_hide) continue;
            auto rect = cmp.rect();
            if (rect.width < 1.0f || rect.height < 1.0f) continue;
            // Skip elements that haven't been laid out yet (default computed =
            // -1)
    ...
    virtual void for_each_with(Entity &e, PendingE2ECommand &cmd,
        ...
        if (cmd.is_consumed()) {
            ...
            if (!cmd.succeeded()) {
                ...
                log_warn("[E2E ERROR] {} (line {}): {}", cmd.name,
                         cmd.line_number, cmd.error_message);
            }
            e.cleanup = true;
            return;
        }

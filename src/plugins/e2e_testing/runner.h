...
constexpr std::array<std::string_view, 16> single_arg_commands = {
    ...
    "right_click_ui", "right_click_text", "expect_draw_calls_below"
};
...
class E2ERunner {
   public:
    ...
    void tick(float dt) {
        ...
        if (draining_pending_ && has_pending_commands()) return;
        draining_pending_ = false;

        if (index_ >= commands_.size()) {
            if (has_pending_commands()) return;
            finalize_current_script();
            finished_ = true;
            return;
        }
   ...
   private:
    void dispatch_command(const ParsedCommand &cmd) {
        ...
        if (cmd.name == "validate" && property_getter_ &&
            cmd.args.size() >= 2) {
            ...
            if (actual == cmd.args[1]) {
                ...
                log_warn(
                    "[E2E ERROR] validate (line {}): Expected {}={}, got "
                    "{}",
                    cmd.line_number, cmd.args[0], cmd.args[1], actual);
                failed_ = true;
                current_script_errors_++;
                pending.consume();  // Mark as handled (failed validation is
                                    // still "consumed")
            }
        }
    ...
    bool has_pending_commands() const {
        ...
        for (const Entity &e : pending) {
            if (!e.get<PendingE2ECommand>().is_consumed()) return true;
        }
        return false;
    }
    ...
    void finalize_current_script() {
        ...
        // Sync with command handler error count (from
        // E2ECommandCleanupSystem)
    ...
    bool draining_pending_ =
        false;  // Waiting for pending commands before next script

...
struct animation : developer::Plugin {
    ...
    struct AnimationManager {
        ...
        void update(float dt) {
            for (auto &kv : tracks) {
                ...
                if (tr.duration <= 0.f) {
                    ...
                    tr.elapsed += dt;
                    float u =
                        apply_ease(tr.current_easing, tr.elapsed / tr.duration);
                    ...
                    if (!tr.watchers.empty()) {
                        for (auto &w : tr.watchers) {
                            ...
                            if (!w.last_value.has_value() ||
                                q != *w.last_value) {
                                w.last_value = q;
                                if (w.callback) w.callback(q);
                            }
                        }
                    }
                    ...
                    if (tr.elapsed >= tr.duration) {
                        ...
                        if (!tr.queue.empty()) {
                            ...
                            tr.active = false;
                            if (tr.on_complete) tr.on_complete();
                        }
                    }
                }
            }
        ...
        std::optional<float> get_value(Key key) const {
            auto it = tracks.find(key);
            if (it == tracks.end() || !it->second.active) return std::nullopt;
    ...
    struct AnimHandle {
        ...
        AnimHandle &sequence(const std::vector<AnimSegment> &segments) {
            ...
            if (!tr.active && tr.queue.empty()) {
                ...
                for (auto &s : segments) tr.queue.push_back(s);
            }
            return *this;
        }
        ...
        AnimHandle &on_step(float step, std::function<void(int)> cb) {
            return on_change(
                [step](float v) {
                    return static_cast<int>(std::floor(v / step));
                },
                std::move(cb));
        }
    ...
    template<typename Key>
    static inline AnimationManager<Key> &manager() {
        static AnimationManager<Key> m;
        return m;
    }
    template<typename Key>
    static inline AnimHandle<Key> anim(Key key) {

...
struct collision : developer::Plugin {
    ...
    struct UpdateCollidingEntities : System<TransformType> {
        ...
        struct Callbacks {
            ...
            std::function<std::optional<int>(const Entity &)>
                get_absorber_parent_id;
            ...
            std::function<bool(const Transform &, const Transform &)>
                check_overlap;
        } callbacks;
        ...
        void positional_correction(Transform &a, Transform &b,
            ...
            float correctionMagnitude = std::max(penetrationDepth, 0.0f) /
                                        (1.0f / a.collision_config.mass +
                                         1.0f / b.collision_config.mass);
            vec2 scaled = {collisionNormal.x * correctionMagnitude,
                           collisionNormal.y * correctionMagnitude};
        ...
        void resolve_collision(Transform &a, Transform &b, const float dt) {
            ...
            vec2 impulseVector = {collisionNormal.x * impulse_magnitude,
                                  collisionNormal.y * impulse_magnitude};

            if (a.collision_config.mass > 0.0f &&
                a.collision_config.mass != std::numeric_limits<float>::max()) {
                float inv_mass_a = 1.0f / a.collision_config.mass;
                a.velocity = {a.velocity.x - impulseVector.x * inv_mass_a,
                              a.velocity.y - impulseVector.y * inv_mass_a};
            }

            if (b.collision_config.mass > 0.0f &&
                b.collision_config.mass != std::numeric_limits<float>::max()) {
                float inv_mass_b = 1.0f / b.collision_config.mass;
                b.velocity = {b.velocity.x + impulseVector.x * inv_mass_b,
                              b.velocity.y + impulseVector.y * inv_mass_b};
            }

            vec2 relativeVelocity = {b.velocity.x - a.velocity.x,
                                     b.velocity.y - a.velocity.y};
            float dot =
                callbacks.dot_product(relativeVelocity, collisionNormal);
            vec2 scaled_normal = {collisionNormal.x * dot,
                                  collisionNormal.y * dot};
            vec2 tangent_vec = {relativeVelocity.x - scaled_normal.x,
                                relativeVelocity.y - scaled_normal.y};
            ...
            float frictionImpulseMagnitude =
                ...
                (1.0f / a.collision_config.mass +
                 1.0f / b.collision_config.mass);
            float frictionCoefficient = std::sqrt(a.collision_config.friction *
                                                  b.collision_config.friction);
            frictionImpulseMagnitude = std::clamp(
                frictionImpulseMagnitude, -impulse * frictionCoefficient,
                impulse * frictionCoefficient);

            float friction_magnitude =
                frictionImpulseMagnitude * collision_scalar * dt;
            vec2 frictionImpulse = {tangent.x * friction_magnitude,
                                    tangent.y * friction_magnitude};

            if (a.collision_config.mass > 0.0f &&
                a.collision_config.mass != std::numeric_limits<float>::max()) {
                float inv_mass_a = 1.0f / a.collision_config.mass;
                a.velocity = {a.velocity.x - frictionImpulse.x * inv_mass_a,
                              a.velocity.y - frictionImpulse.y * inv_mass_a};
            }

            if (b.collision_config.mass > 0.0f &&
                b.collision_config.mass != std::numeric_limits<float>::max()) {
                float inv_mass_b = 1.0f / b.collision_config.mass;
                b.velocity = {b.velocity.x + frictionImpulse.x * inv_mass_b,
                              b.velocity.y + frictionImpulse.y * inv_mass_b};
            }
        ...
        float calculate_penetration_depth(const Transform &a,
                                          const Transform &b) {
            float overlapX =
        ...
        float calculate_dynamic_restitution(const Transform &a,
                                            const Transform &b) {
        ...
        float calculate_impulse(const Transform &a, const Transform &b,
            ...
            impulse /= (1.0f / a.collision_config.mass +
                        1.0f / b.collision_config.mass);
        ...
        virtual void for_each_with(Entity &entity, Transform &transform,
            ...
            if (callbacks.should_skip_entity &&
                callbacks.should_skip_entity(entity)) {
                return;
            }

            if (callbacks.is_floor_overlay &&
                callbacks.is_floor_overlay(entity)) {
                return;
            }
            ...
            for (Entity &other : can_collide) {
                if (callbacks.is_floor_overlay &&
                    callbacks.is_floor_overlay(other)) {
                    continue;
                }
                ...
                if (callbacks.gets_absorbed && callbacks.gets_absorbed(other)) {
                    if (callbacks.get_absorber_parent_id) {
                        auto parent_id =
                            callbacks.get_absorber_parent_id(other);
                        if (parent_id.has_value() &&
                            parent_id.value() == entity.id) {
                            ids.insert(other.id);
                            continue;
                        }
                    }

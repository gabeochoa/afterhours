...
// Render layers. Modals sit well above base UI. Header/content are children
// of the modal, so they render above the panel at the same layer via document
// pre-order (no per-child offset needed).
enum struct RenderLayer : int {
    Base = 0,
    Modal = 1000,
};

// Layers between stacked modals: each nested modal is bumped by this so it
// (and its backdrop, one layer below) clears the modal beneath it. It is a
...
struct modal : developer::Plugin {
    ...
    struct Modal : BaseComponent {
        ...
        // open&)
        void request_close(
            DialogResult close_result = DialogResult::Dismissed) {
            pending_close = true;
    ...
    struct ModalResult {
        ...
        bool confirmed() const {
            return dialog_result == DialogResult::Confirmed;
        }
        bool cancelled() const {
            return dialog_result == DialogResult::Cancelled;
        }
        bool dismissed() const {
            return dialog_result == DialogResult::Dismissed;
        }
    ...
    struct detail {
        ...
        static ModalRoot &get_modal_root() {
            ...
            if (!root) {
                ...
                throw std::runtime_error(
                    "ModalRoot not initialized. Call "
        ...
        // Render layer for a modal at its current depth in the stack: deeper
        // modals render above shallower ones (and their backdrops/content).
        static int render_layer_for_depth(EntityID id, int base_layer) {
            auto &root = get_modal_root();
            auto it = std::find(root.modal_stack.begin(),
                                root.modal_stack.end(), id);
            int depth = (it == root.modal_stack.end())
                            ? 0
                            : static_cast<int>(
                                  std::distance(root.modal_stack.begin(), it));
            return base_layer + depth * MODAL_LAYER_STRIDE;
        }

        // Internal implementation for creating a modal
        static ui::imm::ElementResult modal_impl(
            ...
            // Content helpers read m.render_layer (set here) for the exact
            // layer to sit just above this modal's panel.
            config.render_layer =
                render_layer_for_depth(entity.id, config.render_layer);
            ...
            // Note: the modal is already queued for render by init_component
            // (via apply_render_layer) in correct document order — before its
            // children — so header/content at the same layer render above the
            // panel. No explicit re-queue (which would draw the panel over its
            // own children) and no per-child layer offset are needed.
        ...
        static ModalResult create_fyi_content(
            ui::imm::HasUIContext auto &ctx, ui::imm::EntityParent ep_pair,
            bool &open, const std::string &title, const std::string &message,
            const std::string &primary_label, const std::string &dismiss_label,
    ...
    static bool should_receive_input(EntityID entity_id) {
        auto &root = detail::get_modal_root();
        if (root.modal_stack.empty()) return true;
    ...
    static void close(EntityID modal_id,
        ...
        Entity &entity = opt.asE();
        if (!entity.has<Modal>()) return;

        Modal &m = entity.get<Modal>();
        m.return_value = return_value;
        m.request_close(result);
        // Note: modal_impl will handle stack removal when it processes the
        // close
    }
    ...
    struct ModalCloseWatcherSystem : ui::SystemWithUIContext<Modal> {
        ...
        virtual void once(float) override {
            ...
            if (!top_entity.has<Modal>()) return;
            ...
            if (top_modal.closed_by == ClosedBy::Any && modal_active_on_press &&
                ...
                if (!ui::is_point_inside_entity_tree(top_id, press_pos) &&
                    !ui::is_point_inside_entity_tree(top_id,
                                                     context->mouse.pos)) {
    ...
    struct ModalBackdropRenderSystem : System<> {
        ...
        virtual void once(float) override {
            ...
            for (EntityID modal_id : root.modal_stack) {
                OptEntity opt =
                    ui::UICollectionHolder::getEntityForID(modal_id);
                if (!opt.has_value()) continue;

                Entity &entity = opt.asE();
                if (!entity.has<Modal>()) continue;

                Modal &m = entity.get<Modal>();

                // Draw full-screen backdrop
                draw_rectangle({0, 0, static_cast<float>(screen_w),
    ...
    // Returns ModalResult with dialog_result set to
...
inline modal::ModalResult modal_with_result(
    ui::imm::HasUIContext auto &ctx, ui::imm::EntityParent ep_pair, bool &open,
    ModalConfig config = ModalConfig()) {

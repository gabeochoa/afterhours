...
// If you see a linker error, make sure to compile
// vendor/afterhours/src/plugins/settings.cpp
...
// JSON library
// - If you want to provide your own JSON type, define JSON_TYPE before
// including
//   this header.
...
struct settings : developer::Plugin {
  ...
  template <typename SettingsData> struct ProvidesSettings : BaseComponent {
    SettingsData data;
    std::string settings_file;
    std::vector<std::function<void(const SettingsData &)>> on_change_callbacks;

    ProvidesSettings() = default;
    ProvidesSettings(const SettingsData &default_data, const std::string &file)
  ...
  struct SettingsLoadSystem : System<ProvidesSettings<SettingsData>> {
    virtual void once(float) override {
      auto *provider =
          EntityHelper::get_singleton_cmp<ProvidesSettings<SettingsData>>();
      if (!provider) {
        return;
      }
  ...
  static ProvidesSettings<SettingsData> *get_provider() {
    return EntityHelper::get_singleton_cmp<ProvidesSettings<SettingsData>>();
  }
  ...
  static void init(const std::string & /* game_name */,
    ...
    if (EntityHelper::get_default_collection()
            .has_singleton<ProvidesSettings<SettingsData>>()) {
    ...
    add_singleton_components<SettingsData>(entity, default_data, settings_file);
    EntityHelper::merge_entity_arrays();
  }

  template <typename SettingsData> static SettingsData &get_data() {
    auto *provider = get_provider<SettingsData>();
    if (!provider) {
      log_error("Settings plugin not initialized. Call "
  ...
  template <typename SettingsData> static const SettingsData &get_data_const() {
    auto *provider = get_provider<SettingsData>();
    if (!provider) {
      log_error("Settings plugin not initialized. Call "
                "settings::init<SettingsData>() "
  ...
  template <typename SettingsData>
  static void
  register_on_change(std::function<void(const SettingsData &)> callback) {
    auto *provider = get_provider<SettingsData>();
    if (!provider) {
      log_error("Settings plugin not initialized. Call "
                "settings::init<SettingsData>() "
  ...
  template <typename SettingsData> static bool load() {
    auto *provider = get_provider<SettingsData>();
    if (!provider) {
      log_error("Settings plugin not initialized. Call "
                "settings::init<SettingsData>() "
  ...
  template <typename SettingsData> static bool save() {
    auto *provider = get_provider<SettingsData>();
    if (!provider) {
      log_error("Settings plugin not initialized. Call "
                "settings::init<SettingsData>() "

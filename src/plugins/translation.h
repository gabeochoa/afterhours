...
class TranslatableString {
   public:
    ...
    template<typename ParamNameMap>
    fmt::dynamic_format_arg_store<fmt::format_context> get_params(
        const ParamNameMap &param_map) const {
    ...
    operator std::string() const {
        if (skip_translate()) return underlying_TL_ONLY();
...
struct translation_plugin : developer::Plugin {
    ...
    struct ProvidesTranslation : BaseComponent {
        ...
        const TranslationMap &get_translations_for_language(
            Language lang) const {
        ...
        std::string get_string(StringKeyEnum key) const {
            const auto &trans_map =
                get_translations_for_language(current_language);
            auto it = trans_map.find(key);
            if (it != trans_map.end()) {
                return it->second.get_text();
            }
            log_warn("Translation not found for key: {}",
                     magic_enum::enum_name(key));
            return "MISSING_TRANSLATION";
        }

        TranslatableStringType get_translatable_string(
            StringKeyEnum key) const {
            const auto &trans_map =
                get_translations_for_language(current_language);
            auto it = trans_map.find(key);
            if (it != trans_map.end()) {
    ...
    static void enforce_singletons(SystemManager &sm) {
        sm.register_update_system(
            std::make_unique<
                developer::EnforceSingleton<ProvidesTranslation>>());
    }
    ...
    static void load_cjk_fonts(FontManager &font_manager,
                               const std::string &font_file,
                               FontNameGetter get_font_name,
                               FontIDMapper font_id_mapper) {
        auto *provides = EntityHelper::get_singleton_cmp<ProvidesTranslation>();
        if (!provides) {
            log_error("Translation plugin not initialized");
            return;
        ...
        for (const auto &lang : {Language::Korean, Language::Japanese}) {
            const auto &translations =
                provides->get_translations_for_language(lang);
            ...
            while (pos < all_chars.length()) {
                const unsigned char *bytes =
                    reinterpret_cast<const unsigned char *>(all_chars.c_str() +
                                                            pos);
                int codepoint = 0;
                ...
                if (bytes[0] < 0x80) {
                    ...
                    bytes_consumed = 1;
                           ...
                           pos + 1 < all_chars.length()) {
                    codepoint = ((bytes[0] & 0x1F) << 6) | (bytes[1] & 0x3F);
                    bytes_consumed = 2;
                } else if ((bytes[0] & 0xF0) == 0xE0 &&
                           pos + 2 < all_chars.length()) {
        ...
        if (!all_codepoints.empty()) {
            std::vector<int> codepoints(all_codepoints.begin(),
                                        all_codepoints.end());

            if (!font_file.empty() && codepoints.size() > 0) {
                FontIDEnum korean_font_id = font_id_mapper(Language::Korean);
                FontIDEnum japanese_font_id =
                    font_id_mapper(Language::Japanese);
                std::string korean_font_name = get_font_name(korean_font_id);
                std::string japanese_font_name =
                    get_font_name(japanese_font_id);
                ...
                log_trace(
                    "Loaded CJK fonts with {} total codepoints for Korean and "
                    "Japanese support",
                    codepoints.size());
            } else {
                log_warn(
                    "Cannot load CJK fonts: font_file is empty or no "
                    "codepoints");
            }
        }
    }

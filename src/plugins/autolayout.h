...
namespace ui {
    ...
    static float compute_size_for_standalone_expectation(UIComponent& widget, size_t exp_index) {
    ...
    static float _sum_children_axis_for_child_exp(UIComponent& widget, size_t exp_index) {
    ...
    static float compute_size_for_child_expectation(UIComponent& widget, size_t exp_index) {
        float no_change = widget.computed[exp_index];
    ...
    static void solve_violations(UIComponent& widget) {
        ...
        for(EntityID child : widget.children){
            if(to_cmp(child).absolute) continue;
            num_children++;
        }
        ...
        const auto _solve_error_optional = [&widget](size_t exp_index, float* error) {
        ...
        const auto fix_violating_children = [num_children, &widget](size_t exp_index, float error){
            ...
            */
            ...
            float approx_epc = error / (1.f * std::max(1, (int) num_resizeable_children));
            for (EntityID child : widget.children) {
                UIComponent& child_cmp = to_cmp(child);
                Size exp = child_cmp.desired[exp_index];
        ...
        const auto compute_error = [ACCEPTABLE_ERROR, _total_child, _solve_error_optional, fix_violating_children, &widget](size_t exp_index) -> float {

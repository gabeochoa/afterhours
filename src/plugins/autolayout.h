...
inline Size screen_pct(const float value, const float strictness = 0.9f) {
    ...
    return ui::Size{.dim = ui::Dim::ScreenPercent,
                    .value = value,
...
struct UIComponent : BaseComponent {
    ...
    template<typename T, size_t axes = 2>
    struct AxisArray {
    ...
    Rectangle bounds() const {
        ...
        return Rectangle{
            .x = rect_.x - computed_padd[Axis::left] -
                 computed_margin[Axis::left],
            .y =
                rect_.y - computed_padd[Axis::top] - computed_margin[Axis::top],
            .width =
                rect_.width + computed_padd[Axis::X] + computed_margin[Axis::X],
            .height = rect_.height + computed_padd[Axis::Y] +
                      computed_margin[Axis::Y],
        };
    };
    ...
    auto &add_child(EntityID id_) {
        if (id_ == id) {
            log_error("Adding child with id {} that matches our current id {}",
                      id_, id);
        }
...
struct FontManager : BaseComponent {
    ...
    auto &load_font_with_codepoints(const std::string &font_name,
        ...
        if (!font_file) {
            log_error("Cannot load font '{}' with null file path",
                      font_name.c_str());
            return *this;
        }
    ...
    auto &set_active(const std::string &font_name) {
        if (!fonts.contains(font_name)) {
            log_warn(
                "{} missing from font manager. Did you call load_font() on it "
                "previously?",
                font_name.c_str());
        }
...
inline bool is_dimension_percent_based(
    const UIComponent::AxisArray<Size, 6> &desire, Axis axis) {
...
struct AutoLayout {
    ...
    Entity &to_ent(EntityID id) {
        ...
        if (it == mapping.end()) {
            log_error(
                "during autolayout, we looked for an entity with id {} but it "
    ...
    std::array<float, 4> for_each_spacing(
        UIComponent &widget,
    ...
    void write_each_spacing(
        UIComponent &widget, UIComponent::AxisArray<float, 6> &computed,
        const std::function<float(UIComponent &, Axis)> &cb) {
    ...
    float get_text_size_for_axis(UIComponent &widget, Axis axis) {
        ...
        if (ent.is_missing<HasLabel>()) {
            log_warn(
                "Trying to size a component by Text but component doesnt have "
        ...
        if (external_measure_text) {
            result =
                external_measure_text(font_name, content, font_size, spacing);
        } else {
    ...
    float compute_margin_for_exp(UIComponent &widget, Axis axis) {
        ...
        switch (axis) {
            case Axis::X:
                return compute_(margin[Axis::left]) +
                       compute_(margin[Axis::right]);
            case Axis::Y:
                return compute_(margin[Axis::top]) +
                       compute_(margin[Axis::bottom]);
            case Axis::top:
            case Axis::left:
            case Axis::right:
            case Axis::bottom:
    ...
    void calculate_standalone(UIComponent &widget) {
        widget.computed[Axis::X] =
            compute_size_for_standalone_exp(widget, Axis::X);
        widget.computed[Axis::Y] =
            compute_size_for_standalone_exp(widget, Axis::Y);
        ...
        write_each_spacing(widget, widget.computed_margin,
                           [&](UIComponent &w, Axis a) {
                               return compute_margin_for_exp(w, a);
                           });
    ...
    float compute_size_for_parent_expectation(const UIComponent &widget,
        ...
        if (widget.parent == -1) {
            if (exp.dim == Dim::Percent) {
                log_error(
                    "Trying to compute percent expectation for {}, but never "
                    "set "
                    "parent",
                    widget.id);
            }
        ...
        if (parent.computed[axis] == -1) {
            if (is_dimension_percent_based(widget.desired_padding, axis)) {
                log_error(
                    "Trying to compute expectation for {}, but parent {} size "
                    "hasnt "
                    "been calculated yet",
        ...
        float parent_size =
            (parent.computed[axis] - parent.computed_margin[axis]);
    ...
    float compute_padding_for_parent_exp(UIComponent &widget, Axis axis) {
        ...
        if (widget.parent == -1) {
            if (widget.desired_padding[axis].dim == Dim::Percent) {
                log_error(
                    "Trying to compute padding percent expectation for {}, but "
                    "never set "
                    "parent",
                    widget.id);
            }
    ...
    float compute_margin_for_parent_exp(UIComponent &widget, Axis axis) {
        ...
        if (widget.parent == -1) {
            if (is_dimension_percent_based(widget.desired_padding, axis)) {
                log_error(
                    "Trying to compute margin percent expectation for {}, but "
                    "no parent",
                    widget.id);
            }
            return no_change;
    ...
    float _sum_children_axis_for_child_exp(UIComponent &widget, Axis axis) {
        ...
        for (EntityID entityID : widget.children) {
            ...
            if (cs == -1) {
                ...
                log_error(
                    "expect that all children have been solved by now child {} "
                    "parent {}",
                    entityID, widget.id);
            }
            total_child_size += cs;
    ...
    float _max_child_size(UIComponent &widget, Axis axis) {
        ...
        for (EntityID child_id : widget.children) {
            ...
            if (cs == -1) {
                ...
                // Instead of silently ignoring this, check the cases above
                VALIDATE(false,
                         "expect that all children have been solved by now");
            }
    ...
    float compute_size_for_child_expectation(UIComponent &widget, Axis axis) {
        ...
        if (exp.dim != Dim::Children) return no_change;
    ...
    void tax_refund(UIComponent &widget, Axis axis, float error) {
        ...
        if (num_eligible_children == 0) {
            log_trace(
                "I have all this money to return, but no one wants it :(");
            return;
        }
    ...
    void solve_violations(UIComponent &widget) {
        ...
        for (EntityID child : widget.children) {
            if (this->to_cmp(child).absolute) continue;
            // Ignore anything that should be hidden
            if (this->to_cmp(child).should_hide) continue;
            num_children++;
        }
        ...
        const auto _solve_error_optional = [this, &widget](Axis axis,
            ...
            if (num_optional_children != 0) {
                ...
                for (EntityID child : widget.children) {
                    ...
                    if (child_cmp.absolute) continue;
                    // Ignore anything that should be hidden
                    if (this->to_cmp(child).should_hide) continue;
        ...
        const auto fix_violating_children = [num_children, this, &widget](
                                                Axis axis, float error) {
            ...
            */
            ...
            for (EntityID child : widget.children) {
                ...
                float cur_size = child_cmp.computed[axis];
                child_cmp.computed[axis] =
                    fmaxf(0, cur_size - portion_of_error);
        ...
        const auto compute_error =
            [ACCEPTABLE_ERROR, _total_child, _solve_error_optional,
             fix_violating_children, &widget](Axis axis) -> float {
            ...
            while (error > ACCEPTABLE_ERROR) {
                ...
                if (i_x > 10) {
                    log_trace(
                        "Hit {} iteration limit trying to solve violations {}",
                        axis, error);
                    break;
                }
            }
    ...
    void compute_rect_bounds(UIComponent &widget) {
        ...
        if (widget.parent != -1) {
            UIComponent &parent = this->to_cmp(widget.parent);
            offset = Vector2Type{
                parent.computed_rel[Axis::X] +
                    parent.computed_margin[Axis::left],
                parent.computed_rel[Axis::Y] +
                    parent.computed_margin[Axis::top],
            };

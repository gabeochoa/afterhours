...
struct ComponentConfig {
    ...
    ComponentConfig &with_image_alignment(
        texture_manager::HasTexture::Alignment alignment) {
    ...
    ComponentConfig apply_overrides(const ComponentConfig &overrides) const {
        ...
        if (overrides.has_padding()) merged.padding = overrides.padding;
        if (overrides.has_margin()) merged.margin = overrides.margin;
        if (overrides.has_size_override()) merged.size = overrides.size;
        ...
        if (!overrides.label.empty()) merged.label = overrides.label;

        if (overrides.has_any_rounded_corners())
            merged.rounded_corners = overrides.rounded_corners;

        if (overrides.is_disabled()) merged.disabled = overrides.disabled;
        if (overrides.is_hidden()) merged.hidden = overrides.hidden;
        ...
        if (overrides.is_absolute) merged.is_absolute = overrides.is_absolute;
...
struct UIStylingDefaults {
    ...
    std::optional<ComponentConfig> get_component_config(
    ...
    bool has_component_defaults(ComponentType component_type) const {
        return component_configs.find(component_type) !=
               component_configs.end();
    }
...
ComponentConfig _overwrite_defaults(HasUIContext auto &ctx,
    ...
    if (!config.is_internal) {
        if (styling_defaults.has_component_defaults(component_type)) {
            config =
                styling_defaults.merge_with_defaults(component_type, config);
        }
    }
...
static void apply_flags(Entity &entity, const ComponentConfig &config) {
    ...
    if (config.select_on_focus) entity.addComponentIfMissing<SelectOnFocus>();
...
static void apply_texture(Entity &entity, const ComponentConfig &config) {
    if (!config.texture_config.has_value()) return;

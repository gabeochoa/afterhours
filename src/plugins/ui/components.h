...
struct HasChildrenComponent : BaseComponent {
    ...
    void add_child(Entity &child) {
        ...
        if (on_child_add) on_child_add(child);
    }
...
struct HasDropdownState : ui::HasCheckboxState {
    ...
    HasDropdownState(
        ...
        : HasCheckboxState(false),
          options(opts),
          fetch_options(fetch_opts),
          on_option_changed(opt_changed) {}

    HasDropdownState(
        const std::function<Options(HasDropdownState &)> fetch_opts)
...
struct HasNavigationBarState : ui::HasDropdownState {
    ...
    HasNavigationBarState(
        const std::array<std::string_view, N> &opts,
        const std::function<void(size_t)> opt_changed = nullptr)
        : HasNavigationBarState(Options(opts.begin(), opts.end()),
                                opt_changed) {}
...
struct HasImage : BaseComponent {
    ...
    HasImage(afterhours::texture_manager::Texture tex,
             std::optional<afterhours::texture_manager::Rectangle> src =
                 std::nullopt,

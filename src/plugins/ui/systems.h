...
struct BeginUIContextManager : System<UIContext<InputAction>> {
    ...
    InputBits inputs_as_bits(
        ...
        for (auto &input : inputs) {
            if (input.amount_pressed <= 0.f) continue;
...

        {
            ...
            if (inpc.has_value()) {
                ...
                for (auto &actions_done : inpc.inputs_pressed()) {
                    context.last_action =
                        static_cast<InputAction>(actions_done.action);
                }
            }
        }
...
static void print_debug_autolayout_tree(Entity &entity, UIComponent &cmp,
    ...
    std::cout << "Desired(" << cmp.desired[Axis::X] << ","
              << cmp.desired[Axis::Y] << ") ";

    if (cmp.absolute) std::cout << "[ABS] ";
...
struct TrackIfComponentWillBeRendered : System<> {
    ...
    virtual void for_each_with(Entity &entity, float) override {
        if (entity.is_missing<UIContext<InputAction>>()) return;
...
struct EndUIContextManager : System<UIContext<InputAction>> {
    virtual void for_each_with(Entity &, UIContext<InputAction> &context,
                               float) override {
        if (context.focus_id == context.ROOT) return;
...
template<typename InputAction>
struct ComputeVisualFocusId : System<> {
    virtual void for_each_with(Entity &, float) override {
        ...
        if (ctx->focus_id == ctx->ROOT) return;
        ...
        while (current->has<ui::InFocusCluster>()) {
            ...
            if (cmp.parent < 0 || ++guard > 64) break;
...
struct HandleClicks : SystemWithUIContext<ui::HasClickListener> {
    ...
    virtual void for_each_with(Entity &entity, UIComponent &component,
        ...
        if (component.should_hide || entity.has<ShouldHide>()) return;
   ...
   private:
    void process_derived_children(Entity &parent,
                                  UIComponent &parent_component) {
        ...
        for (auto child_id : ui_component.children) {
            ...
            if (!child_component.was_rendered_to_screen) continue;
...
struct HandleTabbing : SystemWithUIContext<> {
    ...
    virtual void for_each_with(Entity &entity, UIComponent &component,
        ...
        if (entity.is_missing<HasClickListener>() &&
            ...
            return;
        if (entity.has<SkipWhenTabbing>()) return;
        if (entity.has<ShouldHide>()) return;
        if (!component.was_rendered_to_screen) return;
...
struct HandleDrags : SystemWithUIContext<ui::HasDragListener> {
    ...
    virtual void for_each_with(Entity &entity, UIComponent &component,
                               HasDragListener &hasDragListener,
                               float) override {
   ...
   private:
    void process_derived_children(Entity &parent,
        ...
        for (auto child_id : ui_component.children) {
            ...
            Entity &child = child_entity.asE();
            if (!child.has<UIComponent>() ||
                !child.has<ui::HasLeftRightListener>()) {
                continue;
            }
            ...
            if (!context->has_focus(child.id)) continue;
...
// this->include_derived_children = true;
    ...
    if (hasDropdownState.options.size() > 100) {
        log_error(
            "UpdateDropdownOptions: Entity {} - fetch_options returned {} "
            "options - ABORTING",
        ...
        for (size_t i = 0; i < options.size(); i++) {
            ...
            child.addComponent<ui::HasClickListener>([i, &entity](Entity &) {
                ui::HasDropdownState &hds =
                    entity.get_with_child<HasDropdownState>();
                hds.changed_since = true;
                hds.on = !hds.on;

                hds.last_option_clicked = i;
                if (hds.on_option_changed) hds.on_option_changed(i);
            });
    ...
    for (auto child_id : ui_component.children) {
        ...
        if (child_hasChildren.children.size() == 0) {
            ...
            for (size_t i = 0; i < options.size(); i++) {
                ...
                grandchild.addComponent<ui::HasClickListener>(
                    [i, &child](Entity &) {
                        ui::HasDropdownState &hds =
                            child.get_with_child<HasDropdownState>();
                        hds.changed_since = true;
                        hds.on = !hds.on;

...
ElementResult sprite(HasUIContext auto &ctx, EntityParent ep_pair,
    ...
    _init_component(ctx, ep_pair, config, ComponentType::Image, false,
                    "sprite");
...
inline ElementResult image_button(
...
ElementResult icon_row(HasUIContext auto &ctx, EntityParent ep_pair,
    ...
    for (const auto &frame : frames) {
        ...
        sprite(
            ctx, mk(row.ent(), static_cast<int>(i)), spritesheet, frame,
...
ElementResult button(HasUIContext auto &ctx, EntityParent ep_pair,
    ...
    _init_component(ctx, ep_pair, config, ComponentType::Button, true,
                    "button");
    ...
    for (size_t i = 0; i < labels.size(); ++i) {
        if (button(ctx, mk(entity, i),
                   ComponentConfig::inherit_from(
                       config, fmt::format("button group {}", i))
                       .with_size(config.size)
...
ElementResult checkbox_group(
    HasUIContext auto &ctx, EntityParent ep_pair, std::bitset<Size> &values,
...
static std::string generate_label_text(
...
static void update_main_label(Entity &slider_entity,
    ...
    if (!slider_cmp.children.empty()) {
        ...
        std::string new_text =
            generate_label_text(original_label, value, position);
...
ElementResult slider(HasUIContext auto &ctx, EntityParent ep_pair,
    ...
    _init_component(ctx, ep_pair, config, ComponentType::Slider, true,
                    "slider");
    ...
    elem.ent().template get<UIComponent>().set_desired_width(
        config.size.x_axis);
    ...
    auto apply_slider_value = [&](Entity &target, float new_value_pct) {
        ...
        if (clamped == sliderState.value) return;
    ...
    const bool tiny_width =
        ...
        ((dim == Dim::Percent || dim == Dim::ScreenPercent) &&
         width_val < 0.02f);
    if (tiny_width) {
        log_warn(
            "slider width is very small (dim={}, value={:.4f}); slider handle "
            "may be invisible (component: {})",
            (int) dim, width_val, config.debug_name.c_str());
    }

    Size handle_width_size{dim, width_val * 0.25f,
    ...
    slider_bg.addComponentIfMissing<ui::HasLeftRightListener>(
        [apply_slider_value, &sliderState](Entity &ent, int dir) {
            ...
            apply_slider_value(ent,
                               sliderState.value + (dir < 0 ? -step : step));
        });
...
ElementResult pagination(HasUIContext auto &ctx, EntityParent ep_pair,
    ...
    if (options.empty()) return {false, entity};
    ...
    for (size_t i = 0; i < options.size(); i++) {
        if (button(ctx, mk(entity, child_index + i),
                   ComponentConfig::inherit_from(
                       config, fmt::format("option {}", i + 1))
                       .with_size(
                           ComponentSize{pixels(default_component_size.x / 2.f),
                                         config.size.y_axis})
...
ElementResult dropdown(HasUIContext auto &ctx, EntityParent ep_pair,
        ...
        auto label = div(ctx, mk(entity),
                         ComponentConfig::inherit_from(config, "dropdown_label")
                             ...
                             .with_rounded_corners(
                                 RoundedCorners(button_corners).right_sharp())
                             .with_render_layer(config.render_layer + 0));
        label.ent().template addComponentIfMissing<InFocusCluster>();
    }
    ...
    const auto on_option_click = [&](Entity &dd, size_t option_index) {
        ...
        EntityID id =
            entity.get<UIComponent>().children[label_str.empty() ? 0 : 1];
    ...
    auto main_button_label =
        fmt::format("{}{}", current_option, drop_arrow_icon);
    ...
    if (main_btn) {
        dropdownState.on ? on_option_click(entity, 0)

...
void render_tree_node(HasUIContext auto &ctx, Entity &scroll_entity,
  ...
  bool expandable = view_config.is_expandable
                        ? view_config.is_expandable(node.data)
                        : !node.children.empty();
  ...
  auto row_config = ComponentConfig::inherit_from(base_config, "tree_row")
                        .with_size(ComponentSize{
                            percent(1.0f), pixels(view_config.row_height)})
...
ElementResult tree_view(HasUIContext auto &ctx, EntityParent ep_pair,
  ...
  auto &state = init_state<HasTreeViewState>(entity, [&](auto &) {});

  // Scrollable container
  auto scroll_config = ComponentConfig::inherit_from(config, "tree_scroll")

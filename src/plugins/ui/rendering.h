...
static inline float _compute_effective_opacity(const Entity &entity) {
    ...
    while (cur) {
        ...
        if (!cur->has<UIComponent>()) break;
        EntityID pid = cur->get<UIComponent>().parent;
        if (pid < 0 || pid == cur->id) break;
...
static inline void draw_text_in_rect(const ui::FontManager &fm,
    ...
    if (afterhours::ui::text_utils::contains_cjk(text)) {
        ...
        draw_text_ex(fm.get_active_font(), text.c_str(),
                     Vector2Type{sizing.x, sizing.y}, sizing.height, 1.f,
                     color);
    }
}

static inline Vector2Type position_texture(
    texture_manager::Texture, Vector2Type size, RectangleType container,
    ...
    switch (alignment) {
        ...
        case texture_manager::HasTexture::Alignment::Center:
            position = Vector2Type{
                .x = container.x + margin_px.x + (container.width / 2) +
                     (size.x / 2),
                .y = container.y + margin_px.y + (container.height / 2) +
                     (size.y / 2),
            };
            break;
...
static inline void draw_texture_in_rect(
    texture_manager::Texture texture, RectangleType rect,
    texture_manager::HasTexture::Alignment alignment) {
...
struct RenderDebugAutoLayoutRoots : SystemWithUIContext<AutoLayoutRoot> {
    ...
    virtual bool should_run(float dt) override {
        ...
        if (enableCooldown < 0) {
            ...
            for (auto &actions_done : inpc.inputs()) {
                if (static_cast<InputAction>(actions_done.action) ==
                    toggle_action) {
                    enabled = !enabled;
                    break;
                }
    ...
    bool is_descendant_of_isolated(const Entity &entity) const {
        if (!isolate_enabled) return false;
        if (entity.id == isolated_id) return false;
        const Entity *cur = &entity;
        int guard = 0;
        while (cur->has<UIComponent>()) {
            const UIComponent &cur_cmp = cur->get<UIComponent>();
            if (cur_cmp.parent < 0 || ++guard > 64) break;
            if (cur_cmp.parent == isolated_id) return true;
    ...
    void render_me(const Entity &entity) const {
        ...
        const std::string widget_str =
            fmt::format("{:03} (x{:05.2f} y{:05.2f}) w{:05.2f}xh{:05.2f} {}",
                        (int) entity.id, cmp.x(), cmp.y(), cmp.rect().width,
                        cmp.rect().height, component_name);
        ...
        if (is_hovered) {
            draw_rectangle_outline(
                cmp.rect(), color_or_hidden(this->context->theme.from_usage(
                                Theme::Usage::Error)));
            draw_rectangle_outline(cmp.bounds(),
                                   color_or_hidden(colors::UI_BLACK));
            draw_rectangle(debug_label_location,
                           color_or_hidden(colors::UI_BLUE));
        } else {
            draw_rectangle(debug_label_location,
                           color_or_hidden(colors::UI_BLACK));
        }
        ...
        if (is_hovered && (left_released || right_released)) {
            IsolationMode new_mode = left_released
                                         ? IsolationMode::NodeAndDescendants
    ...
    void render_me(const UIContext<InputAction> &context,
        ...
        if (context.visual_focus_id == entity.id) {
            ...
            if (effective_focus_opacity < 1.0f) {
                focus_col =
                    colors::opacity_pct(focus_col, effective_focus_opacity);
            }
            RectangleType focus_rect = cmp.focus_rect();
            if (entity.has<HasUIModifiers>()) {
                focus_rect =
                    entity.get<HasUIModifiers>().apply_modifier(focus_rect);
            }
        ...
        if (entity.has<texture_manager::HasTexture>()) {
            ...
            texture_manager::Rectangle src =
                img.source_rect.value_or(texture_manager::Rectangle{
                    0.0f, 0.0f, (float) img.texture.width,
                    (float) img.texture.height});
    ...
    virtual void for_each_with_derived(const Entity &entity,
        ...
        for (size_t i = 0; i < context.render_cmds.size(); ++i) {
            for (size_t j = i + 1; j < context.render_cmds.size(); ++j) {
                if ((context.render_cmds[i].layer >
                     context.render_cmds[j].layer) ||
                    (context.render_cmds[i].layer ==
                         context.render_cmds[j].layer &&

    ...
    if (entity.has<HasCheckboxMark>()) {
      ...
      if (result.rect.height >= 1.f) {
        ...
        if (per_unit) {
          ...
          instances.reserve(units.spans.size());
          float prefix_w = 0.f;
          size_t prev_end = 0;
          for (size_t i = 0; i < units.spans.size(); ++i) {
            const UnitSpan &sp = units.spans[i];
            if (sp.begin > prev_end)
              prefix_w += measure_text(font, hasLabel.label.substr(prev_end, sp.begin - prev_end).c_str(),
                                       font_size, spacing)
                              .x;
            const UnitDraw d = unit_draw(entity, i);
            const float w = measure_text(font, units.units[i].c_str(), font_size, spacing).x;
            if (d.opacity > 0.f)
              instances.push_back({units.units[i].c_str(), prefix_w + d.x, d.y,
                                   font_size * d.scale, colors::opacity_pct(font_col, d.opacity)});
            prefix_w += w;
            prev_end = sp.begin + units.units[i].size();
          }
          if (!instances.empty())
            buffer.add_text_units(label_rect, instances.data(), instances.size(), resolved,

...
// Define ENABLE_PROFILING to enable profiling, or DEBUG to enable it
// automatically Define RELEASE to disable profiling
...
public:
  ...
  bool init_file(const std::string &filename) {
    ...
    buffer.tid = static_cast<uint32_t>(
  ...
  bool begin_event(const std::string &name, const std::string &args = "") {
    ...
    return spall_buffer_begin_args(
        &ctx, &buffer, name.c_str(), static_cast<int32_t>(name.length()),
        args.c_str(), static_cast<int32_t>(args.length()), get_timestamp());
  }
...
#define PROFILE_END()                                                          \
  if (profiling::g_profiler.is_initialized())                                  \
  profiling::g_profiler.end_event()
#define PROFILE_FLUSH()                                                        \
  if (profiling::g_profiler.is_initialized())                                  \
  profiling::g_profiler.flush()
...
public:
  ...
  bool begin_event(const std::string &, const std::string & = "") {
    return false;
  }

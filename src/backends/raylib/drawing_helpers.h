...
// When rotation is non-zero, uses DrawTextPro to rotate around (centerX,
// centerY) When rotation is zero, uses standard DrawTextEx
inline void draw_text_ex(const raylib::Font font, const char *content,
  ...
  raylib::DrawTextPro(font, content, drawPos, origin, rotation, font_size,
                      spacing, color);
}
...
inline void draw_rectangle_rounded_rotated(
  ...
  RectangleType centeredRect = {-rect.width / 2.0f, -rect.height / 2.0f,
...
inline void pop_rotation() { raylib::rlPopMatrix(); }

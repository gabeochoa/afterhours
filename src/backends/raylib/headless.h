...
struct RaylibHeadless {
  ...
  /// Creates a headless GL context, initializes rlgl, and sets up a render
  /// texture.
  ...
  inline void begin_frame() { raylib::BeginTextureMode(render_texture); }

  /// End rendering to the render texture.
  inline void end_frame() { raylib::EndTextureMode(); }

  /// Clear the background with the specified color.
  inline void clear(raylib::Color color) { raylib::ClearBackground(color); }
  ...
  inline bool capture_frame(const std::filesystem::path &path) {
    ...
    // Both are needed to guarantee the render texture contains final pixel
    // data.
    glFlush();
    glFinish();
  ...
  inline RenderTextureType &get_render_texture() { return render_texture; }
};

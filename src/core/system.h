...
template <typename T> struct is_not_component : std::false_type {};

template <typename T> struct is_not_component<Not<T>> : std::true_type {};

// Helper to unwrap Not<T> to get T
template <typename T> struct unwrap_not {
  using type = T;
};

template <typename T> struct unwrap_not<Not<T>> {
  using type = T;
};
...
template <typename T, typename... Rest> struct filter_components<T, Rest...> {
  ...
  using type =
      std::conditional_t<is_component<T>::value && !is_not_component<T>::value,
...
struct System
  ...
  private:
    template <typename C> static bool check_component(const Entity &entity) {

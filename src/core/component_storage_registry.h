...
struct ComponentStorageRegistry {
  ...
  std::unordered_map<ComponentID, std::unique_ptr<ComponentStorageBase>>
      storages;
  ...
  ComponentStorage<Component> &get_or_create_storage() {
    ...
    if (it != storages.end()) {
      ...
      auto *storage =
          dynamic_cast<ComponentStorage<Component> *>(it->second.get());
      if (storage) {
        return *storage;
      }
  ...
  template <typename Component> ComponentStorage<Component> &get_storage() {
    ComponentID id = components::get_type_id<Component>();
    auto it = storages.find(id);
    if (it == storages.end()) {
  ...
  template <typename Component> bool has_storage() const {
    ComponentID id = components::get_type_id<Component>();
  ...
  void cleanup() {
    // ComponentStorage handles cleanup via remove_component during normal
    // operations This is a no-op but kept for API consistency
  }

  // Clear all storages
  void clear_all() { storages.clear(); }

  // Get total number of component storages
  size_t size() const { return storages.size(); }
};

...
struct Entity {
    ...
    template<typename T>
    [[nodiscard]] bool has_child_of() const {
#if defined(AFTER_HOURS_DEBUG)
    ...
    template<typename T>
    [[nodiscard]] const T &get_with_child() const {
#if defined(AFTER_HOURS_DEBUG)
        log_trace("fetching for child components {} {} on entity {}",
                  components::get_type_id<T>(), type_name<T>(), id);
#endif
...
#pragma GCC diagnostic pop
    ...
    void enableTag(const TagId tag_id) {
        if (tag_id >= AFTER_HOURS_MAX_ENTITY_TAGS) return;
        tags.set(tag_id);
    }
    ...
    [[nodiscard]] bool hasTag(const TagId tag_id) const {
        if (tag_id >= AFTER_HOURS_MAX_ENTITY_TAGS) return false;
...
// Treat ECS reference wrappers as "pointer-like" for pointer-free snapshot
// APIs.
template<>
struct is_pointer_like<RefEntity> : std::true_type {};
template<>
struct is_pointer_like<OptEntity> : std::true_type {};
}  // namespace afterhours
